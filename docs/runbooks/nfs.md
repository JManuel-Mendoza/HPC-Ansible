...
- Export (server): `nfs_export_path` (este repo: `/data/nfs` en `group_vars/all/vars.yml`)
- Mountpoint (clients): `nfs_client_mountpoint` (este repo: `/data/nfs` en `group_vars/all/vars.yml`)
...
df -hT | grep -E ' nfs|/data/nfs' || true
mountpoint -q /data/nfs; echo $?
```
...
- En clientes: desmonta `/data/nfs`, limpia fstab y (si están vacíos) intenta borrar directorios de montaje legacy.
...
# Remover export NFS del master (no borra /data/nfs ni su contenido)
