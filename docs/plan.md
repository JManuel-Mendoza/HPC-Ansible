...
- `ansible-playbook -i inventario.ini base.yml --syntax-check`: falla por `Errno 13` en este entorno de auditoría
...
20. Cerrar gap de `base.yml --syntax-check` en entorno de auditoría.
...
- `KEEP`: entrypoints (`site.yml`, `base.yml`), roles activos bajo `roles/**`, vars de inventario, docs activas y herramientas de auditoría
