...
\centering
\small
\caption{Conceptos de Ansible y ubicación en este proyecto}
\label{tab:ansible_conceptos_donde}
...
\hline
\textbf{Concepto} & \textbf{Dónde se ve en este proyecto} & \textbf{Qué aporta} \\
\hline
Inventario & \texttt{inventario.ini} & Define hosts, grupos y variables de conexión. \\
\hline
Playbook & \texttt{site.yml} & Orquesta etapas y roles sobre grupos del clúster. \\
\hline
Role & \texttt{roles/common/}, \texttt{roles/slurm\_install/}, \texttt{roles/slurm\_validate/} & Encapsula tareas reutilizables por dominio funcional. \\
\hline
\texttt{group\_vars} & \texttt{group\_vars/all/vars.yml}, \texttt{group\_vars/hpc\_master.yml} & Centraliza variables globales y por grupo. \\
\hline
\texttt{host\_vars} & \texttt{host\_vars/master.yml}, \texttt{host\_vars/worker2.yml} & Permite ajustes específicos por nodo. \\
\hline
Vault & \texttt{group\_vars/all/vault.yml} & Guarda variables sensibles cifradas. \\
\hline
Templates & \path{roles/slurm_install/templates/slurm.conf.j2}, \path{roles/nfs_hpc/templates/exports.j2} & Genera archivos de configuración dinámicos con variables. \\
\hline
Handlers & \path{roles/firewall/handlers/main.yml}, \path{roles/users_ssh/handlers/main.yml} & Ejecuta acciones diferidas solo cuando hubo cambios. \\
\hline
\end{tabular}
\end{table}
...
Comprender los módulos es clave porque cada uno tiene parámetros propios y resultados distintos para instalación, configuración y validación del clúster.

\section{Playbook principal del proyecto (\texttt{site.yml})}
\label{sec:ansible_site_base}

El archivo \texttt{site.yml} es el punto de entrada del proyecto de automatización. En el repositorio se utiliza como playbook principal para ejecutar etapas sobre grupos específicos del inventario y aplicar roles en orden controlado.

\begin{yamlcode}
site.yml (extracto representativo)
...
Esta estructura se complementa con etapas adicionales del mismo \texttt{site.yml} para red, almacenamiento, Slurm, entorno LLM y validaciones.
...
\centering
\small
\caption{Precedencia de variables aplicada en el proyecto (de menor a mayor prioridad)}
\label{tab:ansible_precedencia_aplicada}
\begin{tabular}{|c|>{\raggedright\arraybackslash}p{0.32\textwidth}|>{\raggedright\arraybackslash}p{0.52\textwidth}|}
\hline
\textbf{Orden} & \textbf{Ubicación} & \textbf{Ejemplo real del proyecto} \\
\hline
1 & \texttt{roles/nfs\_hpc/defaults/main.yml} & \texttt{nfs\_hpc\_server\_enabled: false} define el valor por defecto del rol. \\
\hline
2 & \texttt{group\_vars/all/vars.yml} & Variables globales para todos los hosts (por ejemplo \texttt{nfs\_export\_path}); no sobrescribe \texttt{nfs\_hpc\_server\_enabled}. \\
\hline
3 & \texttt{group\_vars/hpc\_master.yml} & \texttt{nfs\_hpc\_server\_enabled: true} sobrescribe el default para el grupo \texttt{hpc\_master}. \\
\hline
4 & \texttt{host\_vars/<host>.yml} & Si se define allí, el valor por host tiene prioridad final sobre defaults y group\_vars. \\
\hline
\end{tabular}
\end{table}
...
En este punto, \texttt{worker1} y \texttt{worker2} pueden resolverse por nombre si existe DNS o si ya existen entradas en \texttt{/etc/hosts}. Si no hay resolución de nombres, el inventario se completa asignando \texttt{ansible\_host=<IP>} a cada worker.
