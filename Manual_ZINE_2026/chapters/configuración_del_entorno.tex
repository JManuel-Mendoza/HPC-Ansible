...
Al finalizar la descarga, el archivo ISO quedó disponible localmente con el nombre \path{Rocky-9.x-x86_64-minimal.iso} (o un nombre equivalente si el navegador añadió sufijos), y con un tamaño del orden de gigabytes, coherente con una imagen de instalación. \cite{rocky_9_isos}
...
\begin{enumerate}
  ...
  \item Se descargó el software Ventoy desde su página oficial (\url{https://www.ventoy.net/en/download.html}).
...
\section{Red de administración del clúster}
\label{sec:red_administracion}

Esta sección describe cómo se verificó la conectividad de administración del clúster. En este proyecto se utiliza el término \textit{red de administración} para la red institucional existente que permite acceder por SSH a cada nodo. Esta red es distinta de la \textit{red interna del clúster}, reservada para tráfico exclusivo entre nodos.

\begin{table}[H]
\centering
\caption{Nomenclatura de redes usada en el manual.}
\label{tab:nomenclatura_redes}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.38\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}@{}}
\hline
\textbf{Término} & \textbf{Significado} & \textbf{Uso principal} \\
\hline
\textit{red de administración} & Red institucional de acceso remoto a nodos del clúster. & SSH, instalación inicial y operación remota. \\
\textit{red externa de administración} & Variante para enfatizar que la red de administración es externa al plano interno del clúster. & Contrastar explícitamente con la red interna del clúster. \\
\textit{red interna del clúster} & Red exclusiva entre nodos del clúster para tráfico operativo interno. & Slurm, NFS, automatización y validación entre nodos. \\
\hline
\end{tabular}
\end{table}
...
Esto confirmó dos aspectos operativos: (1) el gateway fue \texttt{10.195.34.1} y (2) la IP del \textit{master} en la red de administración fue \texttt{10.195.34.17}. Además, el texto \texttt{proto dhcp} evidenció que la configuración fue obtenida mediante DHCP (asignación automática de red). \cite{iproute2_ip_route}
...
\label{subsec:dhcp_red_administracion}
...
%   \caption{Evidencia de direccionamiento por DHCP en la red de administración (fragmento de \texttt{ip route} con \texttt{proto dhcp}).}
...
\subsection{Autenticación por llaves SSH (\texorpdfstring{usuario $\rightarrow$ master y master $\rightarrow$ workers}{usuario -> master y master -> workers})}
...
La conectividad de administración (\textit{red de administración}) quedó verificada en el \textit{master}, identificando la interfaz utilizada (\texttt{eno1}) y confirmando la salida por una ruta por defecto válida. Se mantuvo el direccionamiento por DHCP durante la fase inicial para disminuir el riesgo de perder acceso remoto, priorizando estabilidad operativa mientras se avanzaba en la configuración del clúster.
